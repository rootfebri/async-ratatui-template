...
macro_rules! header_static {
  {
    ...
    const $name: [(::reqwest::header::HeaderName, ::reqwest::header::HeaderValue); $(ttlen!($key)+)+0usize] = [
  ...
  };
  {
    ...
    $vis const $name: [(::reqwest::header::HeaderName, ::reqwest::header::HeaderValue); $($crate::ttlen!($key)+)+0usize] = [
      $((::reqwest::header::HeaderName::from_static($key), ::reqwest::header::HeaderValue::from_static($key))),+
    ];
  }
}
