...
impl Default for App {
  fn default() -> Self {
    ...
    let (line_tx @ MpscTx { .. }, line_rx @ MpscRx { .. }) = mpsc::channel(1024);
    ...
    tasks.spawn(input_reader(line_tx, input_tx.subscribe(), event_watcher.clone()));
    tasks.spawn(line_checker(line_rx, bucket_tx, event_watcher.clone(), logs.clone()));
    tasks.spawn(output_writer(bucket_rx, output_tx.subscribe(), event_watcher.clone()));
